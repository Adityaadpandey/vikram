...
router.post("/verify", async (req, res) => {
  try {
    const { phone, otp, armyId } = req.body;

    if (!phone || !otp || !armyId) {
      return res.status(400).json({
        error: "Phone number, OTP, and Army ID are required",
      });
    }
...
router.post("/keys-saved", async (req, res) => {
  try {
    ...
    if (!userId || !armyId || !publicKey || !privateKey || !seedPhrase) {
      return res.status(400).json({
        error:
          "All fields are required (userId, armyId, publicKey, privateKey, seedPhrase)",
      });
    }
...
// Step 4: Decrypt private key for chat
router.post("/decrypt-key", async (req, res) => {
  try {
    const { userId, seedPhrase, phone, armyId } = req.body;

    if (!userId || !seedPhrase || !phone || !armyId) {
      return res.status(400).json({
        error: "All fields are required (userId, seedPhrase, phone, armyId)",
      });
    }
    ...
    // Verify army ID matches
    if (user.armyId !== armyId) {
      return res.status(400).json({ error: "Invalid army ID" });
    }
    ...
    const encryptedPrivateKey = user.keys[0].privateKey;
    ...
    try {
      ...
      res.status(400).json({
        error: "Failed to decrypt private key. Check your seed phrase.",
      });
    }
  } catch (err: any) {
...
// Step 6: Get user's public key for chat
